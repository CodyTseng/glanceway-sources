      ...
      if (entryStat.isDirectory()) {
        ...
        if (fs.existsSync(manifestPath)) {
          try {
            ...
            const ext = "zip";
            const downloadUrl = `https://github.com/${GITHUB_REPO}/raw/refs/heads/${GITHUB_BRANCH}/dist/${namespace}/${entry}/latest.${ext}`;
        ...
        try {
          ...
          const version = latestVersion || manifest.version || "1.0.0";
          const ext = "yaml";
          const downloadUrl = `https://github.com/${GITHUB_REPO}/raw/refs/heads/${GITHUB_BRANCH}/dist/${namespace}/${sourceName}/latest.${ext}`;
          const installUrl = `glanceway://install?url=${encodeURIComponent(downloadUrl)}`;
