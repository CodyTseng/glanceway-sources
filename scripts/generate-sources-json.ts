...
const GITHUB_REPO = "codytseng/glanceway-sources";
...
function getVersionsFromDist(author: string, sourceName: string): string[] {
  ...
  for (const file of files) {
    // Match version patterns like "1.0.0.zip" or "1.0.0.yaml"
    const match = file.match(/^(\d+\.\d+\.\d+)\.(zip|yaml)$/);
    if (match) {
    ...
    for (const entry of entries) {
      ...
      if (entryStat.isDirectory()) {
        ...
        if (fs.existsSync(manifestPath)) {
          try {
            ...
            const sourceVersions: SourceVersion[] = versions.map((version) => ({
              version,
              download_url: `https://github.com/${GITHUB_REPO}/raw/refs/heads/${GITHUB_BRANCH}/dist/${namespace}/${entry}/${version}.zip`,
        ...
        try {
          ...
          const sourceVersions: SourceVersion[] = versions.map((version) => ({
            version,
            download_url: `https://github.com/${GITHUB_REPO}/raw/refs/heads/${GITHUB_BRANCH}/dist/${namespace}/${sourceName}/${version}.yaml`,
            changelog: version === manifest.version ? manifest.changelog : undefined,
          }));
